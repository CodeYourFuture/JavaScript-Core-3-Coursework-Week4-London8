let convertToNewRoman = require("./convert-to-new-roman");

test("returns I if passed 1 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(1);
  // Act
  const expectedOutput = "I";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns III if passed 3 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(3);
  // Act
  const expectedOutput = "III";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns IV if passed 4 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(4);
  // Act
  const expectedOutput = "IV";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns V if passed 5 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(5);
  // Act
  const expectedOutput = "V";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns IX if passed 9 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(9);
  // Act
  const expectedOutput = "IX";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns XIV if passed 14 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(14);
  // Act
  const expectedOutput = "XIV";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns XV if passed 15 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(15);
  // Act
  const expectedOutput = "XV";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns XXII if passed 22 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(22);
  // Act
  const expectedOutput = "XXII";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns XLIV if passed 44 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(44);
  // Act
  const expectedOutput = "XLIV";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns LVIII if passed 58 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(58);
  // Act
  const expectedOutput = "LVIII";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns XCIX if passed 99 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(99);
  // Act
  const expectedOutput = "XCIX";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns C if passed 100 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(100);
  // Act
  const expectedOutput = "C";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns CD if passed 400 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(400);
  // Act
  const expectedOutput = "CD";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns D if passed 500 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(500);
  // Act
  const expectedOutput = "D";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns CM if passed 900 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(900);
  // Act
  const expectedOutput = "CM";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns M if passed 1000 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(1000);
  // Act
  const expectedOutput = "M";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns MCMXCIX if passed 1999 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(1999);
  // Act
  const expectedOutput = "MCMXCIX";
  // Assert
  expect(result).toEqual(expectedOutput);
});

test("returns MMXXII if passed 2022 as an argument", function () {
  // Arrange
  const result = convertToNewRoman(2022);
  // Act
  const expectedOutput = "MMXXII";
  // Assert
  expect(result).toEqual(expectedOutput);
});
